...
class Group:
    ...
    font = pygame.font.SysFont(G.FONT, G.FONT_SIZE)  # TODO: implement font sizes
...
class Widget(Group):
    ...
    # Images in String or Texture class, solid colors in String or Tuple or List
    def set_color(self, c_main="main", c_hover="hover", c_click="click",
                  c_outline="outline", c_font="font", c_line="line"):
        if isinstance(c_main, str): c_main = Texture(c_main)
        if isinstance(c_hover, str): c_hover = Texture(c_hover)
        if isinstance(c_click, str): c_click = Texture(c_click)
        if isinstance(c_outline, str): c_outline = C[c_outline]
        if isinstance(c_font, str): c_font = C[c_font]
        if isinstance(c_line, str): c_line = C[c_line]

        c_main.rescale(self.rect)
        c_hover.rescale(self.rect)
        c_click.rescale(self.rect)
        self.c_main = c_main
        self.c_hover = c_hover
        self.c_click = c_click
...
class Overlay(Widget):
    ...
    def __ui(self):
        ...
        self.topbar.set_color(Texture("topbar"))
...
class Input(Widget):
    ...
    def event(self, mouse, event_list):
        ...
        # This is pretty ugly, but if it's done the other way around, collision doesn't get triggered
        else:
            for event in event_list:
                if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    self.state = "None"
...
class Tick(Widget):
    def __init__(self, rect, group, func=None, outline=1, text="√", start_enabled=False):
        super().__init__(rect, group, func, outline, text)
        self.ticked = start_enabled
        self.hover = False
        self.set_color(c_font="tick", c_click=Texture("tick_click"))
